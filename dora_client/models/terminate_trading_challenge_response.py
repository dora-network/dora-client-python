...
class TerminateTradingChallengeResponse(BaseModel):
    """
    ...
    user_id: UUID
    status: StrictStr
    __properties: ClassVar[List[str]] = ["trading_challenge_id", "user_id", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TERMINATED']):
            raise ValueError("must be one of enum values ('TERMINATED')")
        return value
